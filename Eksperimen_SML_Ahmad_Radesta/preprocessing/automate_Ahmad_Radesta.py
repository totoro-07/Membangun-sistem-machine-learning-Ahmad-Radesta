...
def get_dataset_path() -> str:
    ...
    /Eksperimen_SML_Ahmad_Radesta/Heart Failure Prediction Dataset.csv
    """
    # Folder: .../Eksperimen_SML_Ahmad_Radesta/preprocessing
    current_dir = os.path.dirname(__file__)

    # Folder utama: .../Eksperimen_SML_Ahmad_Radesta
...
def preprocess_pipeline(path: str):
    ...
    if "HeartDisease" not in df.columns:
        raise KeyError("[ERROR] Kolom 'HeartDisease' tidak ditemukan.")
    ...
    print(f"[INFO] Train-test split: {X_train.shape[0]} training samples.")

    X_train_scaled, scaler = scale_features(X_train)
    X_test_scaled = scaler.transform(X_test)

    # Simpan hasil preprocessing dalam folder preprocessing
    output_dir = os.path.dirname(__file__)
    output_path = os.path.join(output_dir, "processed_heart_failure_data.csv")

    pd.DataFrame(X_train_scaled).assign(
        HeartDisease=y_train.values
